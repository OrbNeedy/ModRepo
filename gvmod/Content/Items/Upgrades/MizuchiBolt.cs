...
{
    internal class MizuchiBolt : ModItem
    {
        public override void SetStaticDefaults()
        {
            // DisplayName.SetDefault("Mizuchi Upgrade");
            ...
            if (player.whoAmI == Main.myPlayer)
            {
                foreach (Item item in player.inventory)
                {
                    if (item.ModItem is DartLeader gun)
                    {
                        if (gun.Upgrades[1])
                        {
                            gun.Upgrades[1] = false;
                            Main.NewText("Removed Mizuchi from your gun.");
                            break;
                        }
                        else
                        {
                            gun.Upgrades[1] = true;
                            Main.NewText("Added Mizuchi to your gun.");
                            break;
                        }
                    }
                }
