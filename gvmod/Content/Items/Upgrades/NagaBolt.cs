...
{
    internal class NagaBolt : ModItem
    {
        public override void SetStaticDefaults()
        {
            // DisplayName.SetDefault("Naga Upgrade");
            ...
            if (player.whoAmI == Main.myPlayer)
            {
                foreach (Item item in player.inventory)
                {
                    if (item.ModItem is DartLeader gun)
                    {
                        if (gun.Upgrades[0])
                        {
                            gun.Upgrades[0] = false;
                            Main.NewText("Removed Naga from your gun.");
                            break;
                        }
                        else
                        {
                            gun.Upgrades[0] = true;
                            Main.NewText("Added Naga to your gun.");
                            break;
                        }
                    }
                }
